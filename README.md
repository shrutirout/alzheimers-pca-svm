...
### Step 2: Skip Preprocessing : Processed Data is Already Included

The repository already contains everything produced by `src/preprocess.py` and the first two notebooks:

- `Data/processed/refined_images/` : 442 skull-stripped, 128×128 PNG slices, one per subject
- `Data/processed/mapping.csv` : image paths and labels
- `Data/processed/metadata_processed.csv` : full metadata with CDR mappings
- `Data/processed/X_train.npy`, `X_test.npy`, `y_train.npy`, `y_test.npy` : ready-to-use train/test splits

You do not need the raw Kaggle dataset or the original Excel metadata file. Skip `src/preprocess.py`, `phase1_environment_setup.ipynb`, and `phase2_mri_preprocessing.ipynb` entirely and start directly from Phase 3.
...
| Notebook | What it produces | Required? |
|---|---|---|
| `phase1_environment_setup.ipynb` | `class_distribution.png`, `metadata_processed.csv` | Skip : outputs already in repo |
| `phase2_mri_preprocessing.ipynb` | `X.npy`, `y.npy`, `sample_slices.png` | Skip : processed data already in repo |
| `phase3_pca_reduction.ipynb` | `X_pca.npy`, `pca_model.pkl`, variance curve, eigenbrains | Run this first |
| `phase4_splitting_augmentation.ipynb` | `X_train.npy`, `X_test.npy`, `y_train.npy`, `y_test.npy`, `class_weights.json` | Run second |
| `phase5_svm_training.ipynb` | `svm_linear.pkl`, `svm_rbf.pkl`, `cv_scores.csv` | Run third |
| `phase6_evaluation.ipynb` | `classification_report.txt`, `confusion_matrix.png`, `roc_curves.png`, `pca_projection.png`, `component_importance.png` | Run last |
